...
### Model and Code
Within each folder, there are excel sheets containing the inputs for the simulation as well as the temperature measured from the simulation for the input. The corresponding Ansys Icepak simulation files (compressed .tzr projects are available in [Icepak Simulation Files](/data/simulation/). The MATLAB code then imports the temperatures from the Excel sheet, performs optimization and estimates the model parameters. The MATLAB function within each folder then calculates the temperature response for new input power dissipation via LPLSP. 
